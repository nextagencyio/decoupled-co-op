...
import memberFarmsData from '@/data/mock/farms.json'
...
export function handleMockQuery(body: string): any {
  try {
    ...
    if (query.includes('GetMemberFarms') || query.includes('nodeMemberFarms')) {
      return loadMockData('farms.json')
    }
