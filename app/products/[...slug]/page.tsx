...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!item) {
    return { title: 'Product Not Found | Co Op' }
  }
              ...
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).pricePerUnit && (
                    <div>
                      <dt className="text-sm text-gray-500">Price Per Unit</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).pricePerUnit}</dd>
                    </div>
                  )}
                  {(item as any).unitType && (
                    <div>
                      <dt className="text-sm text-gray-500">Unit Type</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).unitType}</dd>
                    </div>
                  )}
                  {(item as any).season && (
                    <div>
                  ...
                  {(item as any).originFarm && (
                    <div>
                      <dt className="text-sm text-gray-500">Origin Farm</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).originFarm}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
