...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  if (!item) {
    return { title: 'Event Not Found | Co Op' }
  }
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).eventType && (
                    <div>
                      ...
                      <dd className="font-semibold text-gray-900">{(item as any).eventType}</dd>
                    </div>
                  )}
                  {(item as any).admission && (
                    <div>
                      <dt className="text-sm text-gray-500">Admission</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).admission}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
